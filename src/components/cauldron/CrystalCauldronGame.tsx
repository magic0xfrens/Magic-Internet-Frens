...
type Phase = "idle" | "spinning" | "summoned" | "fizzle" | "opening" | "opened";
...
export default function CrystalCauldronGame({ collection, ethUsd, col, nftMinted, nftMax, onBought }: Props) {
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      ...
      // TicketWon logs = crystals summoned to you this tx.
      let wonIds: bigint[] = [];
      try {
        const logs = parseEventLogs({ abi: HOOK_ABI, logs: rcpt.logs, eventName: "TicketWon" });
        wonIds = logs
          .filter((l) => (l.args as { player?: string }).player?.toLowerCase() === address?.toLowerCase())
  ...
  const status = useMemo(() => {
    ...
    if (phase === "summoned") return "✦ Crystal summoned! Open it below.";
  ...
  return (
    <section className="ccg">
      ...
      {/* ── loops (spin power) ── */}
      <div className="ccg-lbl">Spin power — more loops, more chances</div>
      <div className="ccg-seg">
        {LOOPS.map((l) => (
          <button key={l} className={`ccg-segb ${loops === l ? "on" : ""}`} disabled={busy} onClick={() => setLoops(l)}>
            <b>{l}×</b><em>loops</em>
          </button>
        ))}
      </div>
      ...
      {/* ── stake ── */}
      <div className="ccg-seg" style={{ marginTop: 8 }}>
        {STAKES.map((s) => (
          <button key={s} className={`ccg-segb ${stake === s ? "on" : ""}`} disabled={busy} onClick={() => setStake(s)}>
            <b>{s}</b><em>Ξ</em>
          </button>
        ))}
      </div>
      ...
      <div className="ccg-odds">
        <div className="ccg-odds-row"><span>Summon chance</span><span className="ccg-odds-v" style={{ color: col }}>{winPct > 0 ? `${winPct.toFixed(0)}%` : "—"}</span></div>
        <div className="ccg-odds-bar"><div style={{ width: `${Math.min(winPct, 100)}%`, background: col }} /></div>
        <div className="ccg-odds-sub">≈ ${spinCost} in · {loops}× volume · {winPct > 0 ? `${winPct.toFixed(0)}% to summon` : "grind Mana"}</div>
      </div>
...
const css = (col: string) => `
  .ccg { position: relative; border-radius: 16px; padding: 18px 16px 16px;
  ...
  .ccg-stage { position: relative; height: 300px; display: grid; place-items: center; margin-bottom: 8px; border-radius: 14px; overflow: hidden; }
  ...
  .ccg-hero { position: relative; z-index: 2; max-width: 90%; max-height: 90%; object-fit: contain;
  ...
  .ccg-tile { position: relative; border-radius: 12px; overflow: hidden; }
  ...
  .ccg-prize-label { font-family: "Fredoka", sans-serif; font-weight: 600; font-size: 13px; color: ${col}; background: rgba(8,6,15,0.72); padding: 3px 12px; border-radius: 999px; border: 1px solid ${col}66; text-shadow: 0 0 10px ${col}; }
  ...
  .ccg-lbl { font-family: "DM Mono", monospace; font-size: 8.5px; letter-spacing: 0.1em; text-transform: uppercase; color: ${C.mute}; margin-bottom: 6px; opacity: 0.85; }
  .ccg-seg { display: flex; gap: 6px; }
  .ccg-segb { flex: 1; padding: 7px 0; border-radius: 9px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); color: ${C.mute}; cursor: pointer; transition: all 0.15s; font-family: "Fredoka", sans-serif; display: flex; flex-direction: column; align-items: center; line-height: 1.1; }
  .ccg-segb b { font-size: 14px; font-weight: 600; color: ${C.cream}; }
  .ccg-segb em { font-size: 8px; font-style: normal; opacity: 0.7; }
  .ccg-segb:hover:not(:disabled) { border-color: ${col}66; }
  .ccg-segb.on { background: ${col}18; border-color: ${col}; }
  .ccg-segb.on b { color: ${col}; }
  .ccg-segb:disabled { opacity: 0.5; cursor: default; }

  .ccg-odds { margin: 12px 0; }
  ...
  .ccg-cry { position: relative; border-radius: 11px; padding: 7px; background: rgba(8,6,15,0.45); border: 1px solid rgba(255,255,255,0.06); display: flex; flex-direction: column; align-items: center; gap: 5px; }
  ...
  .ccg-open { width: 100%; padding: 4px 0; border-radius: 7px; border: 1px solid ${col}; background: ${col}1c; color: ${col}; font-family: "Fredoka", sans-serif; font-weight: 600; font-size: 11px; cursor: pointer; transition: background 0.15s; }
  .ccg-open:hover:not(:disabled) { background: ${col}33; }
  .ccg-open:disabled { opacity: 0.5; cursor: default; }
  .ccg-rar { font-family: "DM Mono", monospace; font-size: 8px; letter-spacing: 0.06em; text-transform: uppercase; padding: 1px 6px; border-radius: 999px; border: 1px solid; }
