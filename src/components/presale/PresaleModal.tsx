...
export default function PresaleModal({ isOpen, onClose, autoMint = false, initialAmount = 1, onSummoned }: PresaleModalProps) {
  ...
  // Notify parent when the summon confirms.
  useEffect(() => { if (p.finalized) onSummoned?.(); }, [p.finalized, onSummoned]);
      ...
      <div className="pm__modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        {p.confirmed ? (
          <div className="pm__done">
            ...
            <a href="#/mi-frens" className="pm__btn pm__btn--primary" onClick={onClose}>View my frens</a>
          </div>
        ) : busy ? (
          <div className="pm__done">
...
const css = `
  ...
  .pm__modal { position: relative; width: 100%; max-width: 400px; border-radius: 22px; padding: 30px 28px 26px;
  ...
  .pm__close { position: absolute; top: 14px; right: 14px; width: 32px; height: 32px; display: grid; place-items: center;
    border-radius: 9px; cursor: pointer; color: #8f83b8; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); transition: all .18s; }
  ...
  .pm__art-inner { position: relative; width: 158px; border-radius: 16px; overflow: hidden;
    border: 1px solid rgba(213,253,81,0.25); box-shadow: 0 10px 30px rgba(0,0,0,0.4); }
  .pm__art-brand { width: 132px; height: 132px; border-radius: 24px; overflow: hidden;
  ...
  .pm__art-scan { position: absolute; inset: 0; pointer-events: none; border-radius: 16px; overflow: hidden;
  ...
  .pm__art-burst { position: absolute; inset: -10px; pointer-events: none; border-radius: 22px;
  ...
  .pm__reveal-class { font: 700 10px/1 "DM Mono", monospace; letter-spacing: 0.08em; text-transform: uppercase; color: #17112f;
    background: linear-gradient(90deg, #d5fd51, #f5c542); padding: 5px 10px; border-radius: 18px; }
  ...
  .pm__step { width: 38px; height: 38px; border-radius: 10px; font-size: 20px; font-weight: 700; cursor: pointer;
  ...
  .pm__btn { display: block; width: 100%; padding: 15px; border-radius: 14px; cursor: pointer; text-decoration: none; text-align: center;
