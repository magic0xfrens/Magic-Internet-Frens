...
| fixD | X5a HIGH cancelled presale can still ignite | contracts/solidity/cauldron/MiFrensGenesis.sol | igniteCauldron | CLAIMED | add cancelled gate so the refund pot can never be forwarded |
| fixD | X5c HIGH vault swept wei vs quote-denominated divisor | contracts/solidity/cauldron/PoolOps.sol | seedFunding (vaultSwept return only) | CLAIMED | report vaultSwept ONLY on the native branch; crystallizeCollection signature unchanged, registry untouched |
| fixD | X5b LOW sniper calls a nonexistent play() | contracts/solidity/cauldron/LaunchSniper.sol | IGachaPlay.play decl, launch | CLAIMED | adapt caller to the router's real 5-arg play(); router NOT edited |
| fixD | X5d LOW three canonical deployment records | contracts/solidity/deployments/sepolia.json, contracts/solidity/deployments/sepolia-final.json, contracts/solidity/deployments/sepolia-launch.json | n/a | CLAIMED | sepolia-launch.json canonical; other two marked superseded |
| fixD | X5e LOW deploy-script ordering assumption | contracts/solidity/deploy/DeployLaunchSniper.s.sol | run | CLAIMED | assert hook.isOpener(gacha) precondition (hygiene, from a REFUTED finding) |
| fixD | tests | contracts/solidity/test/attacks/X5a_GenesisCancelledIgnite.t.sol, contracts/solidity/test/attacks/X5b_SniperSelectorDead.t.sol, contracts/solidity/test/attacks/X5c_VaultSweptDenomination.t.sol | all | CLAIMED | PoCs copied in, to be inverted to regressions |
