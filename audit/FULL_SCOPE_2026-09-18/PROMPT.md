# MiFrens / Cauldron full-scope red-team and remediation brief

> Revision: 2026-09-18. This replaces the round-45 delta-only prompt.
> Baseline commit at authoring: `71443f2bac9275b54536046ead962b4a65f557c2` on `main`.
...
- Run exploit reproductions only on a local EVM or isolated local fork with test accounts and
...
- Produce repository-scoped reproductions and regression tests, not live exploitation tooling.
  Terms such as attacker, attack, exploit, and red team below describe the local threat model and
  test actors; they do not expand these execution boundaries.
...
Use the smallest capable team and obey the runtime's actual concurrency limit. In the current
environment that means at most four active agents including the orchestrator. Do not emulate the
old 24-agent plan.

- Phase gates are strict. Do not launch hunters until the baseline and function graph validate.
...
- Full reports go under `audit/FULL_SCOPE_2026-09-18/`; agent summaries stay under 15 lines.
...
At the authored baseline the repository contains 66 first-party Solidity/deploy/render files, 241
Solidity test files including 164 attack files, 115 frontend TypeScript/TSX files, API handlers,
indexer code, deployment manifests, and shell/Node deployment tooling. These counts are orientation
only. Phase 0 must re-measure them from the exact review tree.
...
- `scripts/`, `deployments/`, `indexer/`, `api/`, `contracts/abis/`, `src/`, build configuration,
  environment templates, and CI/deployment assumptions.
...
- Claims based only on old audit IDs. Old reports are opened only after the blind hunt for
  reconciliation.
...
Create `audit/FULL_SCOPE_2026-09-18/BASELINE.md` containing commands, versions, exit codes, elapsed
...
FOUNDRY_PROFILE=cauldron forge clean
FOUNDRY_PROFILE=cauldron forge build --sizes
FOUNDRY_PROFILE=cauldron forge test --threads 2 -vv
...
stale `out/` artifacts is invalid; this is why `forge clean` comes first.
...
cluster with two semantic mismatches is re-reviewed before hunting.

Deliver `FUNCTION_GRAPH.md`, machine-readable graph files, `SYSTEM_MAPS.md`, and
`GRAPH_VALIDATION.md`. Hunting starts only after the node universe is complete.

## 7. Phase 2 — full-scope adversarial review

Assign non-overlapping clusters within the available agent cap. Each hunter receives the relevant
...
For every cluster, run both attacker modes:

- Extractor: maximize value out versus value in, repeatably.
- Vandal: permanently or materially block the protocol even when the attack is unprofitable.
...
For every public/external function, record either an attacked property with a PoC, a validated
...
Every Medium+ receives an independent adversarial verification:
...
7. execute one realistic happy-path transaction per user-facing feature on a fork or testnet.
...
Fix confirmed Critical, High, and Medium findings. Fix Low findings when the diff is contained;
otherwise log them. Before editing, claim exact files/functions in `LEDGER.md`. Shared files are
serialized.

For each fix:

1. capture the failing invariant/positive exploit before editing;
...
8. re-hunt the changed neighborhood once with a reviewer blind to the original finding.
...
Run from a fresh build and report all exits, failures and skips:
...
Under `audit/FULL_SCOPE_2026-09-18/` produce:
...
- `SYSTEM_MAPS.md` containing authority, asset, lifecycle, denomination, trust, deployment and event
  overlays
- subsystem hunt reports and executable PoCs
...
- Every public/external node has an attacked property, a meaningful executed assertion, or a named
  gap.
...
- Every Medium+ is independently verified and either fixed with a passing regression or explicitly
