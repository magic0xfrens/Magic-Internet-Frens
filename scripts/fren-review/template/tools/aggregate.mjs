...
for (const jobId of jobIds) {
  ...
  for (const s of (await res.json()).submissions ?? []) {
    ...
    const review = { job: jobId, seat, submission: s.hash, findings: 0, verdicts: {}, formatted: head === "FREN-REVIEW v1" };
    ...
    for (const line of summary.split("\n")) {
      const v = /^\s*([a-z]+):\s*(solid|suspect|exploitable)\s*\|\s*(\d+)/i.exec(line);
...
const ledger = {
  ...
  hunts: reviews.length, skippedUnaccepted: skipped, seats: Object.fromEntries(seats), coverage, issues: out, reviews,
};
...
console.log(`${jobIds.length} jobs, ${reviews.length} accepted hunts, ${seats.size} seats taking part, ${out.length} issues -> ledger/LEDGER.md`);
