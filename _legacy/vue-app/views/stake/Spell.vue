    ...
    <div class="stake-wrap" v-if="stakeInfo">
      <div class="actions-block">
        ...
        <div class="action-form" v-if="isMobileView">
          ...
          <ClaimMimBlock
            class="claim-wrap"
            v-if="isMFrenActive && isMobile"
            :isUnsupportedChain="isUnsupportedChain"
            :isDisableClaimButton="isDisableClaimButton"
            :claimAmount="formatAmount((mainToken as MFrenInfo).claimableAmount)"
      ...
      <div class="stake-info">
        <TotalSupplyBlock
          v-if="isMFrenActive && isMobile && isAdditionalInfo"
        ...
        <div class="row">
          ...
          <TokenRatioBlock
            v-else-if="!isMFrenActive && !isMobile"
            :mainToken="mainToken"
            :stakeToken="stakeToken"
          />
        </div>
...
export default {
  data() {
    return {
      ...
      selectedNetwork: 1,
      mFrenNetworks: [1, 42161, 43114],
      sFreneNetworks: [1],
      stakeInfoArr: null as null | FrenStakeInfo[],
      inputAmount: BigInt(0),
      inputValue: "" as string | bigint,
      isMobile: false,
      isAdditionalInfo: false,
      refresherInfo: {
        refresher: null as unknown as dataRefresher<FrenStakeInfo[]>,
        remainingTime: 0,
        isLoading: false,
        intervalTime: 60,
      } as RefresherInfo<FrenStakeInfo[]>,
    };
  },
  ...
  computed: {
    ...mapGetters({
      chainId: "getChainId",
      account: "getAccount",
      localStakeData: "getFrenStakeData",
    }),
    ...
    isTokenApproved() {
      ...
      if (!this.isUnsupportedChain) return true;
      return (
        (this.toToken as MFrenInfo | SFrenInfo)?.approvedAmount >=
        this.inputAmount
      );
    },
    ...
    availableNetworks() {
      if (this.activeToken === "mFren") return this.mFrenNetworks;
      return this.sFreneNetworks;
    },
    ...
    mainToken() {
      return this.stakeInfo![this.activeToken as "mFren" | "sFren"];
    },
    ...
    specialInfo() {
      const mFrenInfo = [
        {
          text: `Make FREN work for you! Stake your FREN into mFREN! No impermanent
        loss, no loss of governance rights. Take part in the fee sharing
        mechanism of Magic Internet Frens and earn MIF! Find out more`,
          link: "https://docs.magicinternetfrens.xyz/learn/intro/stake/mspell",
          subText: `mFREN automatically earns fees from MIF repayments from all wizards
      ...
      const sFrenInfo = [
        {
          text: `Stake your FREN and gain sFREN. No impermanent loss, no loss of governance rights. Continuously compounding.`,
        },
        {
          text: `After each new deposit, all staked FREN are subject to a 24H lock-up period!`,
        },
      ];

      return this.isMFrenActive ? mFrenInfo : sFrenInfo;
    },
  ...
  watch: {
    ...
    stakeInfoArr: {
      handler() {
        if (this.stakeInfoArr) this.setFrenStakeData(this.stakeInfoArr);
      },
      deep: true,
    },
  },
  ...
  methods: {
    ...
    ...mapMutations({
      ...
      setFrenStakeData: "setFrenStakeData",
    }),
    ...
    changeToken(token: "mFren" | "sFren") {
      localStorage.setItem("FREN_SELECTED_TOKEN", token);
      this.activeToken = token;
      this.activeTab = "stake";
      this.inputValue = "";
      this.$router.push({ name: "StakeFren", query: { token: token } });
    },
    ...
    async createOrUpdateInfo() {
      ...
      try {
        ...
        console.error("Error creating or updating Fren stake info:", error);
      }
    },

    getActiveToken() {
      const activeToken = localStorage.getItem("FREN_SELECTED_TOKEN") as
        | "mFren"
        | "sFren";
      const query = this.$route.query.token as "mFren" | "sFren";
    ...
    createDataRefresher() {
      this.refresherInfo.refresher = new dataRefresher(
        ...
        (updatedData: null | FrenStakeInfo[]) =>
  ...
  async created() {
    ...
    this.checkLocalData();
    await this.createOrUpdateInfo();
    this.setFrenStakeData(this.stakeInfoArr);
  },
