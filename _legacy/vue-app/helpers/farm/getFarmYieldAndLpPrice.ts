...
const FRENAddress = "0x090185f2135308bad17527004364ebcc2d37e5f6";
...
export const getFarmYieldAndLpPrice = async (
  ...
  try {
    if (farmInfo.depositedBalance) {
      const tokenAddress =
        farmInfo.depositedBalance.token0.name === "MIF"
          ? MIMAddress
          : FRENAddress;

      const tokenPrice =
        farmInfo.depositedBalance.token0.name === "MIF" ? mimPrice : spellPrice;
