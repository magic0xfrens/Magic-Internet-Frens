<template>
  <div class="mim-left-to-borrow">
    ...
    <div :class="['info', { 'bera-info': isBeraDesign }]">
      ...
      <div class="title">MIF Left to Mint</div>
    </div>
  ...
  computed: {
    ...
    mimLeftToBorrow(): string {
      return formatLargeSum(
        formatUnits(this.cauldron!.mainParams.mimLeftToBorrow, MIF_DECIMALS)
      );
    },
    ...
    totalMimToBorrow(): string {
      ...
      return formatLargeSum(
        formatUnits(mimLeftToBorrow + totalBorrowed, MIF_DECIMALS)
      );
    },
    ...
    currentFrame(): number {
      ...
      const expandDecimals = parseUnits("1", MIF_DECIMALS);
      ...
      const leftToBorrowPercent = formatUnits(
        parseUnits("100", MIF_DECIMALS) - borrowedPercent,
        MIF_DECIMALS
      );
