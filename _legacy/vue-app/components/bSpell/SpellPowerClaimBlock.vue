<template>
  <div class="spell-power-claim-block">
    ...
    <div class="reward-info" v-if="bFrenInfo">
      <div
        class="row"
        v-for="tokenInfo in bFrenInfo.rewardTokensInfo"
        :key="tokenInfo.name"
      >
...
export default {
  ...
  computed: {
    ...
    isClaimReward() {
      ...
      return this.bFrenInfo.rewardTokensInfo?.reduce((acc, tokenInfo) => {
  ...
  methods: {
    ...
    async claimHandler() {
      try {
        ...
        await getRewards(this.bFrenInfo!.stakeInfo!.contract);
...
.spell-power-claim-block {
  ...
  background: url("@/assets/images/bFren/claim-bg.png") no-repeat;
