<template>
  <div class="unstake-form" v-if="bFrenInfo">
    <div class="inputs-wrap">
      <BaseTokenInput
        :value="inputValue"
        :icon="bFrenInfo.bFren?.icon"
        :name="bFrenInfo.bFren?.name"
        :max="bFrenInfo.stakeInfo?.stakeBalance"
    ...
    <div class="lock-info">
      ...
      <Timer
        small
        padding="0px 4px"
        :endDateTimestamp="bFrenInfo.stakeInfo?.unlockTime"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    async actionHandler() {
      if (!this.bFrenInfo) return false;
      if (this.isActionDisabled) return false;
      if (!this.bFrenInfo.stakeInfo) return false;
    ...
    async unstakeHandler() {
      try {
        ...
        await unStake(this.bFrenInfo!.stakeInfo!.contract, this.inputAmount);
