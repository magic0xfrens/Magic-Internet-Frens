<template>
  <div class="stake-form" v-if="bFrenInfo">
...
import type { AprInfo, BFrenInfo } from "@/helpers/bSpell/types";
...
export default {
  ...
  computed: {
    ...
    isActionDisabled() {
      ...
      if (!this.bFrenInfo.stakeInfo) return true;
    ...
    isTokenApproved() {
      ...
      return this.bFrenInfo.stakeInfo?.approvedAmount >= this.inputAmount;
    },
  ...
  methods: {
    ...
    async approveTokenHandler() {
      ...
      const approve = await approveToken(
        ...
        this.bFrenInfo!.stakeInfo!.contract.address,
        this.inputAmount
      );
    ...
    async stakeHandler() {
      try {
        ...
        await stake(this.bFrenInfo!.stakeInfo!.contract, this.inputAmount);
...
.warning {
  border-radius: 12px;
  border: 1px solid var(--Primary-Gradient, #F7931A);
