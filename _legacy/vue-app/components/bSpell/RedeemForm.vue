<template>
  <div class="redeem-form" v-if="bFrenInfo">
    <div class="inputs-wrap">
      <BaseTokenInput
        :value="inputValue"
        :icon="bFrenInfo.bFren?.icon"
        :name="bFrenInfo.bFren?.name"
        :max="bFrenInfo.bFren?.balance"
        :tokenPrice="bFrenInfo.bFren?.price"
      ...
      <BaseTokenInput
        :value="inputValue"
        :icon="bFrenInfo.spell.icon"
        :name="bFrenInfo.spell.name"
        :tokenPrice="bFrenInfo.spell.price"
        disabled
      />
    </div>
    ...
    <div class="unlock-info" v-if="!isInstantRedeem">
      <img
        class="icon-left"
        src="@/assets/images/bFren/lock-bg-left.png"
        alt=""
      />
      ...
      <img
        class="icon-right"
        src="@/assets/images/bFren/lock-bg-right.png"
        alt=""
      />
    </div>
    ...
    <div class="instant-redeem-info" v-else>
      <img
        ...
        src="@/assets/images/bFren/penalty-left-icon.png"
        alt=""
      />
      ...
      <img
        ...
        src="@/assets/images/bFren/penalty-right-icon.png"
        alt=""
      />
    </div>
...
export default {
  ...
  computed: {
    ...
    isInsufficientBalance() {
      return this.inputAmount > (this.bFrenInfo?.bFren?.balance || 0n);
    },
    ...
    isTokenApproved() {
      ...
      return (
        (this.bFrenInfo?.bFren?.approvedAmount || 0n) >= this.inputAmount
      );
    },
    ...
    unlocksOn() {
      return moment
        .unix(
          Number(moment().unix()) +
            Number(this.bFrenInfo?.lockInfo?.lockDuration || 0)
        )
        .format("DD MMM YYYY");
    },
    ...
    penaltyAmount() {
      const penaltyPercent = Number(
        formatUnits(
          this.bFrenInfo?.lockInfo.instantRedeemParams.immediateBips || 0n,
          PERCENT_PRESITION
        )
      );
  ...
  methods: {
    ...
    async approveTokenHandler() {
      ...
      const approve = await approveToken(
        this.bFrenInfo!.bFren.contract,
        this.bFrenInfo!.tokenBank.address,
        this.inputAmount
      );

      if (approve) this.$emit("updateBFrenInfo");
