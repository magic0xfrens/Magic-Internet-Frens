...
contract MagicInternetTest is ERC20 {
    string public constant domain = "https://www.mifrens.tech";
    ...
    constructor(address admin_) ERC20("Magic Internet Test", "miTEST") {
        admin = admin_;

        _mint(admin, 144_000e18);
    }

    function initialize(
        address frenRewarder_,
        uint256 startTimestamp_
    ) external {
        require(msg.sender == admin, "ONLY ADMIN");
        require(
            frenRewarder_ != address(0) && startTimestamp_ > block.timestamp,
            "BAD PARAMS"
        );
    ...
    function getFrenRewardAmount(uint256 day) public view returns (uint256) {
        ...
        if (day < 21 * 4) return  96396396396396300000;
        if (maxSupply - totalSupply() < 96396396396396300000)
    ...
    function fillFrenRewarder(uint256 day) external {
        ...
        if (!isRewardMinted[day]) {
            ...
            if (amount > 0) {
                ...
                IFrenRewarder(frenRewarder).fill(day, amount);

                emit FillFrensRewarder(msg.sender, day, amount);
            }
        }
    }
}