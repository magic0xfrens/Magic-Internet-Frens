...
contract MagicInternetReferrals{
    ...
    bytes constant private CHARACTERS = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    uint256 constant private CHARACTERS_LENGTH = 62; 
   ...
   function mint() public payable {
    ...
    if (mintETH){
        ...
        if (referedBy[msg.sender] != address(0)) {
            ...
            uint256 remainingReward = mintPriceETH;
...
function signUpReferral(string memory code) public {
     ...
     function generateReferralCode() public returns (string memory) {
        ...
        for (uint256 i = 0; i < 6; i++) {
            uint256 rand = uint256(keccak256(abi.encodePacked(referralCodeCounter, block.difficulty, block.timestamp, i)));
            code[i] = CHARACTERS[rand % CHARACTERS_LENGTH];
        }

        string memory newCode = string(abi.encodePacked("mifren-", string(code)));
