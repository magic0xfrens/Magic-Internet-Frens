#!/usr/bin/env node
/**
 * verify-64-clean.mjs — Full 64x64 test with stale trait-0-6-3.bin excluded.
 * Tests all 7 classes, reports fails per class.
 */
...
for (const f of readdirSync(TRAITS_DIR).filter(f => f.startsWith('trait-') && f.endsWith('.bin'))) {
    // SKIP stale trait-0-6-3.bin
    if (f === 'trait-0-6-3.bin') { console.log('(skipping stale trait-0-6-3.bin)'); continue; }
...
function quantize(canvas, palette, w, h, maxC) {
    const cs = new Set(); for (let i = 0; i < w * h; i++) cs.add(canvas[i]); const uv = Array.from(cs); if (uv.length <= maxC) return;
...
console.log('=== FULL TEST: 64x64 / 16 colors (stale Elf body 3 excluded) ===\n');
let grandFails = 0, grandTotal = 0;
...
for (let ci = 0; ci < 7; ci++) {
    ...
    for (const bi of Object.keys(bodies)) for (const fi of Object.keys(faces)) for (const ii of Object.keys(items)) {
        ...
        if (r > worst) worst = r;
        if (r > LIMIT) { fails++; failList.push({ bi, fi, ii, r }); }
        total++;
    }
    const ok = fails === 0; if (!ok) grandFails += fails; grandTotal += total;
    ...
    for (const f of failList) {
        console.log(`    FAIL: b${f.bi} f${f.fi}(${FACE_LABELS[f.fi] || f.fi}) i${f.ii} → ${f.r} bytes`);
    }
}
console.log(`\n  TOTAL: ${grandFails}/${grandTotal} fails`);
