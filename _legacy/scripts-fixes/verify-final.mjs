#!/usr/bin/env node
/**
 * verify-final.mjs — Test remaining options exhaustively:
 * 1. 64x64 with 8 colors
 * 2. 62x62 with 16 colors
 * 3. 62x62 with 14 colors
 */
...
const configs = [
    { cs: 64, maxC: 8, label: '64x64 / 8 colors' },
    { cs: 62, maxC: 16, label: '62x62 / 16 colors' },
    { cs: 62, maxC: 14, label: '62x62 / 14 colors' },
    { cs: 60, maxC: 16, label: '60x60 / 16 colors' },
];

for (const cfg of configs) {
    const { cs, maxC, label } = cfg;
    const cx = Math.floor((120 - cs) / 2);
    console.log(`\n=== ${label} (offset ${cx}) ===`);
    let allOk = true, gf = 0, gt = 0;
    for (let ci = 0; ci < 7; ci++) {
        ...
        const ok=fails===0;if(!ok)allOk=false;gf+=fails;gt+=total;
        console.log(`  ${NAMES[ci].padEnd(12)} worst=${worst} margin=${LIMIT-worst>=0?'+':''}${LIMIT-worst} fails=${fails}/${total} ${ok?'OK':'FAIL'}`);
    }
    console.log(`  TOTAL: ${gf}/${gt} fails — ${allOk?'ALL FIT':'SOME FAIL'}`);
